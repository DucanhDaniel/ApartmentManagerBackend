...
const HouseholdManager: React.FC = () => {
  ...
  // Member State (Local to the opened modal)
  ...
  // Form data for EDITING a member
  ...
  const confirmDelete = async () => {
    ...
    if (deleteConfirmation.type === 'HOUSEHOLD') {
      try {
        ...
        // Since we don't have a real DELETE API for members yet (as per prompt),
        // we will simulate it in the UI list.
        setHouseholdMembers(householdMembers.filter(m => m.id !== deleteConfirmation.id));
        
        // Optimistically update household member count if needed
        ...
        alert("Đã xóa khỏi danh sách hiển thị (API xóa chưa được tích hợp).");
    }
  ...
  const handleAdd = () => {
    setFormData({
      ...
      ownerName: '',
      area: 0,
      memberCount: 0, // Default 0, updated via Member Management
      phoneNumber: '',
      email: '',
      building: '',
      floor: 0,
  ...
  const initiateEditMember = (member: ResidentMember) => {
      ...
      setEditMemberFormData({
          ...
          email: member.email || '', // Explicitly handle undefined to ensure input works
          status: member.status || 'THUONG_TRU',
          newRoomNumber: '' // Initialize as empty
      });
  ...
  const handleUpdateMember = async (e: React.FormEvent) => {
      ...
      try {
          // If newRoomNumber is provided, the backend logic moves the member.
          // This means they should be removed from the *current* list.
          ...
          if (editMemberFormData.newRoomNumber && editMemberFormData.newRoomNumber.trim() !== '') {
              // Remove from current list because they moved
