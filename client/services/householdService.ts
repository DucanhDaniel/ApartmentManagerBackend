import { Household, ResidentMember, ApartmentStatus, ApartmentType, Page, ResidentInfo } from '../types';
...
export const getAllResidents = async (page: number = 0, size: number = 10, search: string = ''): Promise<Page<ResidentInfo>> => {
    ...
    const response = await fetch(`${API_BASE_URL}/residents?${params.toString()}`, {
        method: 'GET',
        headers: getHeaders(),
        credentials: 'include'
    });
...
export const getHouseholdMembers = async (householdId: string): Promise<ResidentMember[]> => {
    try {
...
export const addHouseholdMember = async (householdId: string, member: Partial<ResidentMember>): Promise<ResidentMember> => {
    const payload = {
...
export const updateHouseholdMember = async (memberId: string, member: Partial<ResidentMember> & { newRoomNumber?: string }): Promise<ResidentMember> => {
    const payload = {
        name: member.fullName,
        phoneNumber: member.phoneNumber,
        dob: member.dateOfBirth,
...
export const createHousehold = async (household: Partial<Household>): Promise<Household> => {
