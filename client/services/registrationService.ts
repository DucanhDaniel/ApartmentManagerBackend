...
import { Registration, Page, CreateRegistrationRequest, RegistrationStatus } from '../types';
...
export const getRegistrations = async (
  ...
  const response = await fetch(`${API_BASE_URL}/registrations?${params.toString()}`, {
    method: 'GET',
    headers: getHeaders(),
    credentials: 'include'
  });
...
export const createRegistration = async (request: CreateRegistrationRequest): Promise<any> => {
...
export const updateRegistration = async (id: number, request: CreateRegistrationRequest): Promise<any> => {
