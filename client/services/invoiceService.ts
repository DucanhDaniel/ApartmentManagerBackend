...
export const getInvoices = async (page = 0, size = 10, search = '', status = ''): Promise<Page<Invoice>> => {
...
export const updateInvoiceDetailQuantity = async (id: number, quantity: number): Promise<InvoiceDetail> => {
  const response = await fetch(`${API_BASE_URL}/invoices/details/${id}?quantity=${quantity}`, {
    method: 'PUT',
    headers: getHeaders(),
    credentials: 'include'
  });
...
export const updateInvoiceDueDate = async (id: number, dueDate: string): Promise<Invoice> => {
  const response = await fetch(`${API_BASE_URL}/invoices/${id}?dueDate=${dueDate}`, {
    method: 'PUT',
    headers: getHeaders(),
    credentials: 'include'
  });
...
export const simulatePayment = async (invoiceId: number, amount?: number): Promise<string> => {
