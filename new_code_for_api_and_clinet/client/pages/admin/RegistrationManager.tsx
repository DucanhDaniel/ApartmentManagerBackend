...
import { getRegistrations, approveRegistration, createRegistration, updateRegistration, deleteRegistration } from '../../services/registrationService';
import { getAllResidents, getHouseholds } from '../../services/householdService';
...
const RegistrationManager: React.FC = () => {
  ...
  // Searchable Resident Select State
  ...
  const handleOpenEdit = (reg: Registration) => {
    loadCreationData();
    setEditingId(reg.id);
    // Find resident from local list or create mock if not loaded yet
    const res = residents.find(r => r.id === reg.residentId);
    if (res) {
        setSelectedResident(res);
    } else {
        // Fallback for UI display until full list is loaded
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl max-w-xl w-full p-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
               <h2 className="text-xl font-black text-gray-900 flex items-center"><MapPin className="w-5 h-5 mr-2 text-blue-600" /> {editingId ? 'Cập nhật đăng ký' : 'Đăng ký hộ cư dân'}</h2>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
               <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Cư dân đăng ký</label>
                    <div className="relative" ref={residentDropdownRef}>
                      ...
                      {isResidentDropdownOpen && (
                        <div className="absolute z-[100] mt-2 w-full bg-white border border-gray-200 rounded-xl shadow-2xl max-h-60 overflow-y-auto animate-in fade-in slide-in-from-top-2">
                          {filteredResidents.length > 0 ? (
                            filteredResidents.map((r) => (
                              ...
                              >
                                ...
                                <span className="px-2 py-1 bg-white border border-blue-200 text-blue-600 text-xs font-black rounded-lg">
                                  P.{r.roomNumber}
                                </span>
                              </button>
                            ))
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Loại hình</label>
                        <select 
                     ...
                     <div>
                        <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Trạng thái</label>
                        <div className="px-3 py-2.5 bg-green-50 text-green-700 font-black rounded-xl border border-green-100 flex items-center gap-2 text-sm">
                  ...
                  <div>
                    <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Lý do đăng ký</label>
                    <textarea 
                      required
               ...
               <button 
                ...
                className="w-full py-3 bg-blue-600 text-white rounded-xl font-black shadow-lg shadow-blue-200 hover:bg-blue-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
               >
